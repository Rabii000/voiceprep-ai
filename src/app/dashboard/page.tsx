...
const stats = [
  { label: 'Sessions Done', value: '12', icon: Mic, color: 'text-[#4F46E5]', bg: 'bg-[#4F46E5]/10 dark:bg-[#4F46E5]/20' },
  { label: 'Avg. Score', value: '74', icon: BarChart3, color: 'text-[#10B981]', bg: 'bg-[#10B981]/10 dark:bg-[#10B981]/20' },
  { label: 'Day Streak', value: '5', icon: Flame, color: 'text-[#F59E0B]', bg: 'bg-[#F59E0B]/10 dark:bg-[#F59E0B]/20' },
  { label: 'Hours Practiced', value: '8.5', icon: Clock, color: 'text-[#6366f1]', bg: 'bg-[#6366f1]/10 dark:bg-[#6366f1]/20' },
]

const quickActions = [
  { label: 'Fluency Coach', icon: BookOpen, href: '/fluency', color: 'text-[#4F46E5]', bg: 'bg-[#4F46E5]/10' },
  { label: 'Shadow Speaking', icon: Repeat2, href: '/shadow', color: 'text-[#10B981]', bg: 'bg-[#10B981]/10' },
  { label: 'Quick Drill', icon: Zap, href: '/drill', color: 'text-[#F59E0B]', bg: 'bg-[#F59E0B]/10' },
  { label: 'Answer Library', icon: Star, href: '/library', color: 'text-[#6366f1]', bg: 'bg-[#6366f1]/10' },
  { label: 'Countdown', icon: Calendar, href: '/countdown', color: 'text-[#EF4444]', bg: 'bg-[#EF4444]/10' },
  { label: 'Progress', icon: TrendingUp, href: '/dashboard', color: 'text-slate-500', bg: 'bg-slate-100 dark:bg-slate-700' },
]
...
function ScoreBadge({ score }: { score: number }) {
  ...
  return <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${cls}`}>{score}</span>
}
      ...
      <div className="p-5 lg:p-8 max-w-6xl mx-auto">
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">You have an interview next Tuesday. Let's stay sharp.</p>
          </div>
          <Link href="/session">
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800">
              ...
              <Link href="/dashboard" className="text-xs text-[#4F46E5] hover:underline font-medium">View all</Link>
            </div>
                ...
                <Link key={session.id} href={`/scorecard/${session.id}`}>
                  <div className="flex items-center gap-4 px-5 py-3.5 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-400">{session.company} · {session.date} · {session.duration}</p>
                    </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-bold text-slate-900 dark:text-white">Score Trend</h2>
                <span className="ml-auto text-xs font-semibold text-[#10B981]">↑ +23 pts</span>
              </div>
              <div className="flex items-end gap-1.5 h-16 mb-2">
                {[55, 62, 58, 65, 71, 69, 78].map((v, i) => (
                  <div
                    key={i}
                    className={`flex-1 rounded-t-sm transition-all ${i === 6 ? 'bg-[#4F46E5]' : 'bg-[#4F46E5]/20 dark:bg-[#4F46E5]/15'}`}
                    style={{ height: `${v}%` }}
                  />
                ))}
              </div>
            ...
            <div className="rounded-2xl border border-[#4F46E5]/20 bg-gradient-to-br from-[#4F46E5]/5 to-[#10B981]/5 dark:from-[#4F46E5]/10 dark:to-[#10B981]/5 p-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-sm font-bold text-slate-900 dark:text-white">Coach Tip</h3>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed mb-3">
                Your STAR results are often vague. Before your next session, end every answer with a number — churn %, revenue, time saved.
              </p>
              <Link href="/session">
